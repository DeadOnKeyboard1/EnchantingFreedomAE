...
$thirdParty = Join-Path $projectRoot 'THIRD_PARTY_NOTICES.md'

foreach ($file in $ini, $license, $notice, $thirdParty) {
...
$output = Join-Path $resolvedOutput 'EnchantingFreedomAE-1.3.0-AE-1.6.1170-1.7.104.zip'
if (Test-Path -LiteralPath $output) {
    if ($Force) { Remove-Item -LiteralPath $output -Force }
    else { throw "Refusing to overwrite existing release: $output" }
}
...
try {
    ...
    Copy-Item -LiteralPath $thirdParty -Destination (Join-Path $docs 'THIRD_PARTY_NOTICES.txt')
